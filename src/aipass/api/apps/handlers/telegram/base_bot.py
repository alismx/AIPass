...
# ===================AIPASS====================
# META DATA HEADER
# Name: base_bot.py - BaseBot class for Telegram multi-bot architecture
# Date: 2026-02-24
# Version: 1.2.0
# Category: api/handlers/telegram
#
# CHANGELOG (Max 5 entries):
#   - v1.3.0 (2026-02-26): Integrate LogStreamer for system log streaming to Telegram chat
#   - v1.2.0 (2026-02-24): Automated BotFather creation via Telethon, manual token fallback
#   - v1.1.0 (2026-02-24): Add /create chat @branch command, /status registry info, conversation state
#   - v1.0.0 (2026-02-24): Initial - stdlib-only BaseBot with polling, tmux injection, heartbeat, hooks
#
# CODE STANDARDS:
#   - Uses Prax get_direct_logger() — no event pipeline (FPLAN-0382 migration)
#   - Class-based: BaseBot is both runnable and inheritable
#   - Imports standard commands from telegram_standards.py
#   - Imports file detection from file_handler.py (no duplication)
# =============================================
...
from aipass.api.apps.handlers.telegram.telegram_standards import (  # noqa: F401
    parse_command,
...
from aipass.api.apps.handlers.telegram.file_handler import (
    detect_file_type,
    build_file_prompt,
)
from aipass.api.apps.handlers.telegram.bot_factory import (
    create_bot,
    validate_branch,
    validate_token,
)
...
from aipass.api.apps.handlers.telegram.botfather_client import (
    create_bot_via_botfather,
    check_telethon_setup,
)
from aipass.api.apps.handlers.telegram.log_streamer import LogStreamer
...
class BaseBot:
    ...
    def __init__(
        ...
        # Logger (Prax direct logger — no event pipeline, avoids recursion with log streamer)
    ...
    def run(self) -> int:
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("%s starting (bot_id=%s)", self.bot_name, self.bot_id)

        # Verify connection
        if not self.verify_connection():
            self.logger.error("Startup health check FAILED - cannot reach Telegram API")
            return 1

        self.logger.info("Connected to Telegram API")
        self._health["started_at"] = datetime.now().isoformat()

        # Check for existing lock
        if self._check_lock():
            self.logger.error("Another instance of bot-%s is already running", self.bot_id)
            return 1
        ...
        self.logger.info("Starting poll loop (offset=%d)", offset)
        ...
        while self.state["running"]:
            try:
                ...
                self.logger.error("Error in poll loop: %s: %s", type(e).__name__, e)
    ...
    def verify_connection(self, timeout: int = 15) -> bool:
        ...
        try:
            ...
            if data.get("ok"):
                ...
                self.logger.info(
                    "Telegram API OK - @%s", bot_info.get("username", "unknown")
                )
                return True

            self.logger.error(
                "Telegram API rejected: %s", data.get("description", "unknown")
            )
            return False

        except URLError as e:
            self.logger.error("Telegram API connection failed: %s", e)
            return False
        except Exception as e:
            self.logger.error("Telegram API health check error: %s", e)
            return False
    ...
    def poll_updates(self, offset: int) -> list:
        ...
        try:
            ...
            self.logger.error("Unexpected poll error: %s", e)
            return []
    ...
    def send_message(
        ...
        for attempt in range(3):
            try:
                ...
                self.logger.warning(
                    "sendMessage error (attempt %d): %s", attempt + 1, e
                )

            if attempt < 2:
                time.sleep(1.0 * (2 ** attempt))

        self.logger.error("sendMessage failed after 3 attempts")
    ...
    def edit_message(self, chat_id: int, message_id: int, text: str) -> bool:
        ...
        try:
            ...
            self.logger.warning("editMessageText error: %s", e)
            return False
    ...
    def process_update(self, update: dict) -> None:
        ...
        if self._active_chat_id is None and chat_id:
            ...
            if self.branch_name is not None and self._log_streamer is None:
                self._log_streamer = LogStreamer(
                    self.bot_token, chat_id, self.branch_name
                )
        ...
        if not self.is_user_allowed(user_id):
            self.logger.warning(
                "Blocked message from unauthorized user_id: %s (@%s)", user_id, username
            )
            return

        # Rate limit check
        if not self.check_rate_limit(user_id):
            self.logger.warning("Rate limited user_id: %s", user_id)
            self.send_message(
                chat_id, "Rate limit exceeded. Please wait before sending more messages."
            )
            return
        ...
        if text:
            ...
            if parsed is not None:
                ...
                if cmd_name == "status":
                    ...
                    self.logger.info("Handled /status command")
                    return
                ...
                if result is not None:
                    if isinstance(result, tuple):
                        ...
                        if action == "new":
                            ...
                            self.logger.info("Handled /new command - session killed")
                        return
                    else:
                        self.send_message(chat_id, result)
                        self.logger.info("Handled /%s command", cmd_name)
                        return
        ...
        if text:
            ...
            self.logger.info("Ignoring unsupported message type")
    ...
    def handle_message(self, chat_id: int, text: str, message: dict) -> None:
        ...
        self.logger.info("Processing message (msg_id=%d)", message_id)

        # Ensure tmux session
        if not self.ensure_tmux_session():
            self.logger.error("Cannot process message - tmux session unavailable")
            self.send_message(
                chat_id, "Failed to start Claude session. Check logs."
            )
            return
        ...
        processing_msg_id = (
            processing_result.get("message_id") if processing_result else None
        )

        # Write pending file
        if not self.write_pending_file(chat_id, message_id, processing_msg_id):
            self.logger.error("Failed to write pending file")
        ...
        if not self.inject_message(prompt):
            self.logger.error("Failed to inject message into tmux")
            self._stop_heartbeat()
            self.pending_file.unlink(missing_ok=True)
            self.send_message(
                chat_id, "Failed to send message to Claude session."
            )
            return

        self.logger.info("Message processed successfully (msg_id=%d)", message_id)
    ...
    def handle_file(self, chat_id: int, message: dict) -> None:
        ...
        if photo_list:
            ...
            self.logger.info("Document from user: %s (%d bytes)", filename, file_size)
        ...
        prompt = build_file_prompt(
            file_path, file_type, caption=caption or None, sender_name="Patrick"
        )
        ...
        # Ensure tmux session
        if not self.ensure_tmux_session():
            self.logger.error("Cannot process file - tmux session unavailable")
        ...
        # Send processing indicator
        processing_result = self.send_message(
            chat_id, f"Processing {file_type} file..."
        )
        processing_msg_id = (
            processing_result.get("message_id") if processing_result else None
        )
        ...
        # Write pending file
        if not self.write_pending_file(chat_id, message_id, processing_msg_id):
            self.logger.error("Failed to write pending file for file upload")
            self.send_message(chat_id, "Internal error writing pending file.")
            return
        ...
        # Inject into tmux
        if not self.inject_message(prompt):
            self.logger.error("Failed to inject file message into tmux")
            self._stop_heartbeat()
            self.pending_file.unlink(missing_ok=True)
            self.send_message(chat_id, "Failed to send file to Claude session.")
            return

        self.logger.info("File processed successfully (msg_id=%d)", message_id)

    def _download_file(
        self, file_id: str, filename: Optional[str] = None
    ) -> Optional[Path]:
        """
        ...
        try:
            ...
            self.logger.error("getFile API failed: %s", e)
            return None

        if not data.get("ok"):
            self.logger.error(
                "getFile error: %s", data.get("description", "unknown")
            )
            return None
        ...
        if not file_path_remote:
            self.logger.error("No file_path in getFile response")
            return None

        if file_size > MAX_FILE_SIZE:
            self.logger.warning(
                "File too large: %d bytes (max %d)", file_size, MAX_FILE_SIZE
            )
            return None

        # Step 2: Download
        download_url = (
            f"https://api.telegram.org/file/bot{self.bot_token}/{file_path_remote}"
        )
        ...
        try:
            ...
            self.logger.info("Downloaded file to %s (%d bytes)", dest, file_size)
            return dest
        except Exception as e:
    ...
    def _handle_create_command(self, chat_id: int, args: str) -> None:
        ...
        # Validate branch exists in BRANCH_REGISTRY.json
        branch_info = validate_branch(branch_name)
        if not branch_info:
            self.send_message(
                chat_id,
                f"Branch '@{branch_name}' not found in BRANCH_REGISTRY.json.\n\n"
    ...
    def _handle_create_automated(self, chat_id: int, branch_name: str, branch_path: str) -> None:
        ...
        status_line = "Bot is running!" if auto_started else (
        ...
        self.logger.info(
            ...
            bot_username, branch_name, auto_started,
        )
    ...
    def check_rate_limit(self, user_id: int) -> bool:
        ...
        self._rate_limit_tracker[user_id] = [
            ts
            for ts in self._rate_limit_tracker[user_id]
    ...
    def ensure_tmux_session(self) -> bool:
        ...
        if not self.work_dir.is_dir():
            self.logger.error(
                "work_dir does not exist: %s — refusing to create tmux session", self.work_dir
            )
            return False

        self.logger.info(
            "Creating tmux session '%s' at %s", self.session_name, self.work_dir
        )
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    f"export AIPASS_BOT_ID={self.bot_id}", "Enter",
                ],
                capture_output=True,
            )
            time.sleep(0.3)
            ...
            self.logger.info(
                "tmux session created, waiting 5s for Claude to initialize..."
            )
            time.sleep(5)
            ...
            self.logger.error("tmux not found - is it installed?")
            return False
    ...
    def inject_message(self, text: str) -> bool:
        ...
        try:
            ...
            self.logger.info("Message injected into tmux session")
            return True
        except subprocess.CalledProcessError as e:
            self.logger.error(
    ...
    def _kill_tmux_session(self) -> bool:
        ...
        if not self._tmux_session_exists():
            self.logger.info(
                "tmux session '%s' not running, nothing to kill", self.session_name
            )
            return True
        ...
        try:
            ...
            self.logger.info("Killed tmux session '%s'", self.session_name)
            return True
        except subprocess.CalledProcessError as e:
            self.logger.error(
    ...
    def write_pending_file(
        self, chat_id: int, message_id: int, processing_message_id: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.info("Pending file written for message %d", message_id)
            return True
        except OSError as e:
            self.logger.error("Failed to write pending file: %s", e)
            return False
    ...
    def clean_stale_pending(self) -> None:
        ...
        try:
            ...
            if age > PENDING_TTL and not self._tmux_session_exists():
                ...
                self.logger.info("Cleaned stale pending file (%.0fs old)", age)
        except OSError as e:
            self.logger.warning("Failed to clean stale pending file: %s", e)
    ...
    def _get_transcript_line_count(self) -> int:
        ...
        try:
            ...
            self.logger.warning("Could not read transcript for line count: %s", e)
            return 0
    ...
    def _start_heartbeat(self, chat_id: int, processing_msg_id: int) -> None:
        ...
        def _heartbeat_loop():
            ...
            while not self._heartbeat_stop.is_set():
                ...
                self.edit_message(
                    chat_id, processing_msg_id, f"Processing... ({elapsed_str})"
                )

        self._heartbeat_thread = threading.Thread(
            target=_heartbeat_loop, daemon=True, name=f"heartbeat-{self.bot_id}"
        )
    ...
    def _create_lock(self) -> None:
        ...
        try:
            ...
            self.logger.info("Lock file created: %s", self._lock_file)
        except OSError as e:
            self.logger.error("Failed to create lock file: %s", e)
    ...
    def _remove_lock(self) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to remove lock file: %s", e)
    ...
    def _check_lock(self) -> bool:
        ...
        try:
            lock_data = json.loads(
                self._lock_file.read_text(encoding="utf-8")
            )
            ...
            if pid:
                try:
                    ...
                    self.logger.info("Cleaning stale lock (PID %d is dead)", pid)
                ...
                try:
                    ...
                    pass  # /proc not available, trust PID check
    ...
    def _save_offset(self, offset: int) -> None:
        ...
        try:
            with open(self._offset_file, "w", encoding="utf-8") as f:
                json.dump(
                    {"offset": offset, "updated": datetime.now().isoformat()}, f
                )
        except OSError as e:
            self.logger.error("Failed to save offset: %s", e)
