...
```bash
drone @ai_mail send @dev_central "PRODUCTION STOPPED: FPLAN-{number}" "Issue: [description]. Attempted: [what was tried]. Awaiting guidance."
```
...
- [ ] Status email sent to DEV_CENTRAL:
  ```bash
  drone @ai_mail send @dev_central "FPLAN-{number} Complete" "Summary of what was done, any issues, outcomes"
  ```
