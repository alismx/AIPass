...
class TestApplyMigrations:
    ...
    def test_key_rename(self):
        """key_rename should move a key's value to the new key name."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations
        ...
        migrations = [
            {
                "id": "m001",
                "operations": [
                    {"type": "key_rename", "from": "old_key", "to": "new_key"},
                ],
            },
        ]

        result = apply_migrations(data, migrations)
    ...
    def test_key_rename_skip_if_target_exists(self):
        """key_rename should skip if target key already exists."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations

        data = {"old_key": "old_value", "new_key": "already_here"}
        migrations = [
            {
                "id": "m001",
                "operations": [
        ...
        # Target preserved, source untouched
        assert result["new_key"] == "already_here"
        assert result["old_key"] == "old_value"

    def test_add_missing_keys(self):
        """add_missing_keys should add keys that don't exist."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations
                ...
                "operations": [
                    {
                        ...
                        "parent": "metadata",
                        "keys": {"schema_version": "1.0.0", "type": "config"},
                    },
                ],
            },
        ]
        ...
        assert result["metadata"]["version"] == "1.0"  # preserved
        assert result["metadata"]["schema_version"] == "1.0.0"  # added
        assert result["metadata"]["type"] == "config"  # added

    def test_add_missing_keys_skip_existing(self):
        """add_missing_keys should not overwrite existing keys."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations

        data = {"metadata": {"version": "1.0", "type": "custom"}}
        migrations = [
            {
                "id": "m002",
                "operations": [
        ...
        assert result["metadata"]["type"] == "custom"  # not overwritten

    def test_move_to_nested(self):
        """move_to_nested should move top-level keys under a parent."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations

        data = {"name": "test", "path": "/some/path", "keep": "yes"}
        migrations = [
            {
                "id": "m003",
                "operations": [
                    {
                        "type": "move_to_nested",
                        "source_keys": ["name", "path"],
                        "target_parent": "branch_info",
                    },
                ],
            },
        ]
        ...
        assert "branch_info" in result
        assert result["branch_info"]["name"] == "test"
        assert result["branch_info"]["path"] == "/some/path"
        assert "name" not in result  # moved
        assert "path" not in result  # moved
        assert result["keep"] == "yes"  # untouched

    def test_multiple_operations_in_sequence(self):
        """Multiple operations should apply in order."""
        from aipass.spawn.apps.handlers.json_ops import apply_migrations

        data = {"metadata": {"ver": "1.0"}, "extra": "data"}
        migrations = [
            {
                "id": "m004",
                "operations": [
                    {"type": "key_rename", "from": "metadata.ver", "to": "metadata.version"},
                    {
                        "type": "add_missing_keys",
                        "parent": "metadata",
                        "keys": {"schema": "1.0.0"},
        ...
        assert "ver" not in result["metadata"]
        assert result["metadata"]["schema"] == "1.0.0"
